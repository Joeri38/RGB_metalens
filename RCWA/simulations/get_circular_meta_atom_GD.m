...
substrate_layer_t = 0e-6; % [m]
superstrate_layer_t = 0e-6; % [m]
transmitted_layer_t = 0e-6; % [m]
diameter_range = input.diameter_range;
N_diameter = length(diameter_range);
debug = input.debug;
        ...
        for i_thickness = 1:N_thickness
            ...
            for i_period = 1:N_period
                ...
                for i_diameter = 1:N_diameter
                    this_diameter = diameter_range(i_diameter);
...
% all_meta_atoms = removevars(all_meta_atoms,{'lambda','nbackground','amplitude_TE','amplitude_TM','n_eff','result','max_real_n_eff'});
...
%        completed_jobs.result{i} = out.result;
%         fprintf('done! Time = %g s\n',duration);
    end
end
...
function completed_jobs = run_table_parfor(all_jobs)
 ...
 %       if ~exist(mydir,'dir')   
  ...
  %      cd(mydir);
    ...
    %    completed_jobs.result{i} = output_all{i}.result;
    end
end
