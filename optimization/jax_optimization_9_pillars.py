...
def predict(x, model):
  ...
  print(f'x to be predicted: {x.shape}')
  pred = model(x.reshape(-1, 9).astype('float16')).reshape(x.shape[0], x.shape[1])
...
print(f"9 pillars, radius={radius}um, f={f}um, sep={sep}um, lamb={lamb}um")
...
# turn x_profile of shape (n, n) into x_profile of shape (n, n, 9) with diameters repeated
expanded_array = x_profile[:, :, np.newaxis]
x_profile = np.repeat(expanded_array, 9, axis=2)

# Load neural network
model_1 = GaussianNoiseMLPNet(num_features=9, num_outputs=1, num_hidden_1=256, num_hidden_2=256, num_hidden_3=256)
model_1.load_state_dict(torch.load('neural_networks/9_pillars/lamb_0.488um.pth'))
...
models_jax = {'model_1': model_1, 'model_2': model_2_jax,
              'model_3': model_3_jax}
...
# Save profile
np.save(path + f'metalenses/9_pillars/x_profile_delta={delta}um.npy', x_profile)
