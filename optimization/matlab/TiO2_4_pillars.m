...
input.period_range = 0.5e-6; % [m] OG: 0.64e-6
...
total_tic = tic;
[all_meta_atoms,completed_jobs] = get_4_square_meta_atom_GD(input);
...
subplot(1,2,1);
plot(input.diameter_2_range*1e6,phase(1:res), 'o');
hold on;
plot(input.diameter_2_range*1e6,phase_TM(1:res), 'o');
...
subplot(1,2,2);
plot(input.diameter_2_range*1e6,transmission(1:res).^2, 'o');
hold on;
plot(input.diameter_2_range*1e6,transmission_TM(1:res).^2, 'o');
