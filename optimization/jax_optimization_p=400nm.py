...
phase_matching_init = ((np.load(path + f'metalenses/single_pillar/init_diameters_delta={delta}um_p=400nm_t=600nm_res=200.npy') - 0.05) / 0.35) * make_circle_mask(radius, n, delta)
...
# Load neural network
model_1 = tf.keras.models.load_model(path + 'neural_networks/single_pillar/lamb_0.488um_p=400nm_t=600nm_res=200.keras')
model_1_jax = jax2tf.call_tf(model_1)
model_2 = tf.keras.models.load_model(path + 'neural_networks/single_pillar/lamb_0.532um_p=400nm_t=600nm_res=200.keras')
model_2_jax = jax2tf.call_tf(model_2)
model_3 = tf.keras.models.load_model(path + 'neural_networks/single_pillar/lamb_0.658um_p=400nm_t=600nm_res=200.keras')
...
# Save profile
np.save(path + f'metalenses/single_pillar/x_profile_delta={delta}um_p=400nm_t=600nm.npy', x_profile)
...
for i, lamb in enumerate(params['lamb']):
    ...
    plt.colorbar()
    plt.savefig(path + f'figs/single_pillar/x_profile_lamb={lamb}um_delta={delta}um_p=400nm_t=600nm.png')
    plt.clf()
    
    plt.imshow(field[2000:2100, 2000:2100], cmap='plasma')
    plt.colorbar()
    plt.savefig(path + f'figs/single_pillar/field_lamb={lamb}um_delta={delta}um_p=400nm_t=600nm.png')
    plt.clf()

    plt.imshow(np.mod(np.angle(z), 2*np.pi)*circle_mask, cmap='plasma')
    plt.colorbar()
    plt.savefig(path + f'figs/single_pillar/phase_lamb={lamb}um_delta={delta}um_p=400nm_t=600nm.png')
    plt.clf()

    plt.imshow(np.abs(z)**2, cmap='plasma')
    plt.colorbar()
    plt.savefig(path + f'figs/single_pillar/transmission_lamb={lamb}um_delta={delta}um_p=400nm_t=600nm.png')
    plt.clf()

    plt.imshow(z_real, cmap='plasma')
    plt.colorbar()
    plt.savefig(path + f'figs/single_pillar/z_real_lamb={lamb}um_delta={delta}um_p=400nm_t=600nm.png')
    plt.clf()

    plt.imshow(z_imag, cmap='plasma')
    plt.colorbar()
    plt.savefig(path + f'figs/single_pillar/z_imag_lamb={lamb}um_delta={delta}um_p=400nm_t=600nm.png')
    plt.clf()

    np.save(path + f'figs/single_pillar/field_lamb={lamb}um_delta={delta}um_p=400nm_t=600nm.npy', field)
